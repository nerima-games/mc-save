 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-save the allowed set is EMPTY, and that is not an oversight.
 * mc-save is a tier-1 stable library (plan.md §2.2): a versioned-codec toolkit
 * that knows how to persist things without knowing what things are. Its only
 * org dependency is mc-kernel, which needs no entry because kernel is
 * universally importable (rule 4). If mc-save ever needs to import mc-worldgen
 * in order to persist a chunk, the layering has been inverted — worldgen
 * defines its own format *using* this toolkit, not the other way round.
 * ---------------------------------------------------------------------------
 */
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-save',
